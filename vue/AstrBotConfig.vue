  ...
  <v-dialog v-model="dialog" fullscreen transition="dialog-bottom-transition" scrollable>
    <v-card>
      <v-toolbar color="primary" dark>
        ...
        <v-toolbar-title>编辑 - {{ currentEditingKey }}</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
...
function openEditorDialog(key, iterable, theme, language) {
  currentEditingKey.value = key
  currentEditingValue.value = iterable[key] || ''
  currentEditingLanguage.value = language || 'json'
  currentEditingTheme.value = theme || 'vs-light'
  currentEditingKeyIterable = iterable
  dialog.value = true
}

function saveEditedContent() {
  if (currentEditingKeyIterable && currentEditingKey.value) {
    currentEditingKeyIterable[currentEditingKey.value] = currentEditingValue.value
  }
  dialog.value = false
}
</script>
